...
import io.mtech.model.LoginBean;
import io.mtech.util.DBConnection;
...
public class LoginDao {
	...
	public String authenticateUser(LoginBean loginBean) {
		...
		String passwordDB = "";
		try {
			con = DBConnection.getInstance().getConnection(); // establishing connection
			statement = con.createStatement(); // Statement is used to write queries. Read more about it.
			resultSet = statement.executeQuery("select userName,password from users"); 
			// Here table name is users and
	     	// userName,password are																				// columns. fetching all the
